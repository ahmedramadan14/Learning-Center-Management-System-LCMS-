...
const validateUpdatePayment = (req, res, next) => {
  const body = req.body || {};
  const fieldError = rejectUnexpectedFields(body, ["subscriptionId", "cycleStart", "cycleEnd", "amountDue"]);
...
const validateWaivePayment = (req, res, next) => {
  const body = req.body || {};
  const fieldError = rejectUnexpectedFields(body, ["waivedBy", "waivedReason"]);

  if (fieldError) {
    return next(fieldError);
  }

  const idError = validateObjectId(body.waivedBy, "waivedBy", true);
  if (idError) {
    return next(idError);
  }

  if (typeof body.waivedReason !== "string" || body.waivedReason.trim().length === 0) {
    return next(new ApiError("waivedReason is required.", 400));
  }
...
const validatePaymentList = (req, res, next) => {
  ...
  if (status !== undefined && !PAYMENT_STATUSES.includes(status)) {
    return next(new ApiError("status must be unpaid, partial, paid, or waived.", 400));
  }
...
const validateRemindPayment = (req, res, next) => {
  const body = req.body || {};
  const fieldError = rejectUnexpectedFields(body, ["createdBy"]);
  if (fieldError) {
    return next(fieldError);
  }
  
  const idError = validateObjectId(body.createdBy, "createdBy");
  return idError ? next(idError) : next();
};
