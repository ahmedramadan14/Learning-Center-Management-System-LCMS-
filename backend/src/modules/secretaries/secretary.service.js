...
const createSecretary = async (data) => {
  const { name, email, phone, password, teacherId, teacher, department, permissions, isActive } = data;
...
const updateOneSecretary = async (id, data, currentUser) => {
  ...
  if (data.name || data.phone || data.email || data.isActive !== undefined || data.password) {
    const updates = {
  ...
  return await Secretary.findByIdAndUpdate(
    id,
    {
      ...
      ...(data.permissions && { permissions: data.permissions, permission: undefined }),
    },
